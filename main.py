...
def download_one(symbol: str, period: str = "2y") -> Tuple[Optional[pd.DataFrame], str]:
    try:
        ...
        if missing:
            return None, "OHLCV 컬럼 부족: " + ",".join(missing)
        ...
        return df, f"OK {len(df)}행"
    except Exception as e:
        return None, f"{type(e).__name__}: {e}"


def load_data_bank(assets: List[Asset], period: str = "2y") -> Tuple[Dict[str, pd.DataFrame], pd.DataFrame]:
    bank: Dict[str, pd.DataFrame] = {}
    statuses = []
    for i, a in enumerate(assets, 1):
        print(f"[DATA] {i}/{len(assets)} {a.symbol}")
        df, msg = download_one(a.symbol, period=period)
...
def score_fast(row: pd.Series, pattern: str, asset: Asset) -> Tuple[float, dict]:
    parts = {}
    pattern_score = {"전고점·박스권 돌파": 24, "눌림목 재상승": 22, "정배열 추세": 20, "바닥 다지기 회복": 18, "패턴 약함": 4}.get(pattern, 4)
...
def analyze_fast(asset: Asset, df: pd.DataFrame) -> FastResult:
    ...
    if block:
        ...
        verdict = VERDICT_BLOCK
    return FastResult(now_text(), asset.symbol, asset.name, asset.asset_type, asset.market, asset.theme, verdict, score, pattern, round(float(row["close"]), 4), round(float(row["trading_value"]), 2), round(float(tv_ratio), 2), round(float(stop), 4), round(float(target), 4), round(risk * 100, 2), round(reward * 100, 2), round(float(rr), 2), ", ".join(block), parts["pattern"], parts["trend"], parts["value"], parts["risk"], "빠른 필터 결과")


def check_command(args) -> None:
    assets = load_universe(Path(args.universe))
    _, status_df = load_data_bank(assets, period=args.period)
...
def scan_command(args) -> None:
    assets = load_universe(Path(args.universe))
    bank, status_df = load_data_bank(assets, period=args.period)
    rows: List[dict] = []
    for a in assets:
        df = bank.get(a.symbol)
        if df is None:
            msg = status_df.loc[status_df["symbol"] == a.symbol, "message"]
            rows.append({"checked_at": now_text(), "symbol": a.symbol, "name": a.name, "asset_type": a.asset_type, "market": a.market, "theme": a.theme, "verdict": VERDICT_ERROR, "score": 0, "pattern": "-", "close": 0, "trading_value": 0, "trading_value_ratio20": 0, "stop": 0, "target": 0, "risk_pct": 0, "reward_pct": 0, "rr": 0, "blockers": str(msg.iloc[0]) if len(msg) else "데이터 없음", "pattern_score": 0, "trend_score": 0, "value_score": 0, "risk_score": 0, "memo": "데이터 오류"})
            continue
        try:
            r = analyze_fast(a, df)
            ...
            rows.append({"checked_at": now_text(), "symbol": a.symbol, "name": a.name, "asset_type": a.asset_type, "market": a.market, "theme": a.theme, "verdict": VERDICT_ERROR, "score": 0, "pattern": "-", "close": 0, "trading_value": 0, "trading_value_ratio20": 0, "stop": 0, "target": 0, "risk_pct": 0, "reward_pct": 0, "rr": 0, "blockers": f"{type(e).__name__}: {e}", "pattern_score": 0, "trend_score": 0, "value_score": 0, "risk_score": 0, "memo": "분석 오류"})
...
def make_vector(df: pd.DataFrame, end_idx: int, window: int) -> Optional[np.ndarray]:
    ...
    last = w.iloc[-1]
    extras = np.array([last["ret5"], last["ret20"], last["ret60"], last["close"] / last["ma20"] - 1 if last["ma20"] else 0, last["close"] / last["ma60"] - 1 if last["ma60"] else 0, np.log((last["trading_value"] + 1) / (last["tv20"] + 1)) if last["tv20"] else 0])
...
def select_for_similarity(fast: pd.DataFrame, max_candidates: int) -> pd.DataFrame:
    ...
    mask = fast["verdict"].isin([VERDICT_CANDIDATE, VERDICT_WATCH]) | (fast["score"] >= 65) | ((fast["score"] >= 55) & (fast["trading_value_ratio20"] >= 1.2))
...
def final_decision(row: pd.Series, stats: dict, args) -> Tuple[str, str]:
    fast_score = float(row.get("score", 0) or 0)
    blockers_text = str(row.get("blockers", "") or "")
    rr = float(row.get("rr", 0) or 0)
    tv_ratio = float(row.get("trading_value_ratio20", 0) or 0)
    ...
    reasons = [f"조건식 {fast_score:.1f}점", f"유사사례 {stats['count']}건", f"평균유사도 {stats['avg_similarity']:.1f}%", f"20일상승확률 {stats['win_rate']:.1f}%", f"20일평균수익 {stats['avg_ret']:+.1f}%", f"손익비 {rr:.2f}", f"거래대금20일비 {tv_ratio:.2f}배"]
    buy_score = float(getattr(args, "buy_score", 70))
    watch_score = float(getattr(args, "watch_score", 45))
    if has_blocker:
    ...
    if fast_score >= buy_score and win_ok and ret_ok and rr_ok and tv_ok:
        return FINAL_BUY, " / ".join(reasons) + " / 조건식+유사통계+리스크 통과"
    if fast_score >= watch_score or (win_ok and ret_ok):
...
def similar_command(args) -> None:
    ...
    bank, _ = load_data_bank(assets, period=args.period)
    results = []
    case_rows = []
    for _, cand in candidates.iterrows():
        ...
        for other_symbol, other_df in bank.items():
            ...
            if other_asset is None or other_asset.asset_type != asset.asset_type or other_asset.market != asset.market:
                continue
            ...
            for end_idx in range(args.window - 1, last_end, args.step):
                ...
                cases.append({"target_symbol": symbol, "target_name": cand["name"], "case_symbol": other_symbol, "case_name": other_asset.name, "case_date": str(ind.index[end_idx].date()), "similarity": sim, "ret20": round(ret, 2), "max_up20": round(max_up, 2), "max_down20": round(max_down, 2)})
        ...
        if cases:
            ...
            stats = {"count": len(cases), "win_rate": round(float(np.mean(ret_arr > 0) * 100), 2), "avg_ret": round(float(np.mean(ret_arr)), 2), "avg_max_up": round(float(np.mean(max_up_arr)), 2), "avg_max_down": round(float(np.mean(max_down_arr)), 2), "avg_similarity": round(float(np.mean(sim_arr)), 2), "top_similarity": round(float(np.max(sim_arr)), 2)}
            direction = "상승우위" if stats["win_rate"] >= 60 and stats["avg_ret"] >= 3 else ("하락위험" if stats["win_rate"] <= 45 or stats["avg_ret"] < 0 else "혼조")
        ...
        results.append({"symbol": symbol, "name": cand["name"], "fast_verdict": cand["verdict"], "final_verdict": final_verdict, "fast_score": cand["score"], "pattern": cand["pattern"], "trading_value_ratio20": cand["trading_value_ratio20"], "rr": cand["rr"], "blockers": cand.get("blockers", ""), "similar_case_count": stats["count"], "avg_similarity": stats["avg_similarity"], "top_similarity": stats["top_similarity"], "win_rate20": stats["win_rate"], "avg_ret20": stats["avg_ret"], "avg_max_up20": stats["avg_max_up"], "avg_max_down20": stats["avg_max_down"], "direction": direction, "top_cases": top_cases, "decision_reason": decision_reason})
    out = pd.DataFrame(results)
    if not out.empty:
        order = {FINAL_BUY: 0, FINAL_WATCH: 1, FINAL_RISK_BLOCK: 2, FINAL_WEAK: 3}
